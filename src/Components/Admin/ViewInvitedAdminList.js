...
const ViewInvitedAdminList= ()=> {

  const [userData,setUserData]= useState([]);

  //to get data when the application loads
  useEffect( () =>{
    axios.get("http://localhost:3001/inviteuser").then(
      (response)=>{
   ...
   const GetData =(id)=>{
    axios.get(`http://localhost:3001/inviteuser/${id}`).then(
      (response)=>{
  ...
  const onDelete =(id)=>{
    axios.delete(`http://localhost:3001/inviteuser/delete/${id}`).then(
      (response)=>{
  ...
  //to delete all users
  const DeleteAll = () =>{
    axios.delete("http://localhost:3001/inviteuser")
  }
    ...
    return(
        ...
        <Card style={{padding:"30px 100px 10px 100px"}}>
            <CardBody>
                <CardTitle tag="h5">All Invited Admin Users</CardTitle>
                <CardSubtitle className="mb-2 text-muted" tag="h6">Overview of Invited Users</CardSubtitle>
                <div>
                      <Button className="no-wrap mt-3">Invite a new User</Button>
...
export default ViewInvitedAdminList;